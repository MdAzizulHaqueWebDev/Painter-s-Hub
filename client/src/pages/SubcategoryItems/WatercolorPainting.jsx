...
const WatercolorPainting = () => {
	...
	const [watercolourPainting, setWatercolourPainting] = useState(null);
	useEffect(() => {
		fetch("https://assign-ment-10-server-side.vercel.app/watercolour-painting")
			.then((res) => res.json())
			.then((data) => {
				setWatercolourPainting(data);
		...
		<>
			...
			<div className="text-center my-6 playfair">
				<h2 className="text-3xl font-bold ">
					{" "}
					Watercolor Painting Arts And Crafts
				</h2>
			</div>
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
				{watercolourPainting?.map((data) => (
